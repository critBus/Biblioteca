...
def register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            user=form.save()
            add_user_to_group(username=user.username,
                              group_name=NOMBRE_ROL_SUSCRIPTOR
                              )
            # Aquí puedes añadir una lógica adicional, como enviar un correo de confirmación
            return HttpResponseRedirect("/admin/")  # Redirige a la página de inicio de sesión después de registrarse
    else:
